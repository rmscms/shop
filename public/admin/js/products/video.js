...
document.addEventListener('DOMContentLoaded', function() {
    ...
    function loadLibraryVideos() {
        ...
        fetch(window.RMS.videos.list + '?search=' + encodeURIComponent(search) + '&page=' + currentPage)
            .then(r => r.json())
            .then(data => {
    ...
    // Set main video
    document.querySelectorAll('.set-main-video-btn').forEach(btn => {
    ...
    // Detach video
    document.querySelectorAll('.detach-video-btn').forEach(btn => {
        btn.addEventListener('click', function() {
            const videoId = this.dataset.videoId;
            ...
            if (!window.confirmAction) {
                ...
                window.confirmAction('آیا از جدا کردن این ویدیو مطمئن هستید؟', function() {
